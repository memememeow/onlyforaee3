...
int main (int argc, char **argv) {
  ...
  unsigned int name_len = (unsigned int)(strlen(target_name) + 1);
  int blocks_needed = name_len / EXT2_BLOCK_SIZE +
                      (name_len % EXT2_BLOCK_SIZE != 0);
  if (argc == 5) { // Create soft link
    ...
    while ((block_index + 1) * EXT2_BLOCK_SIZE < name_len) {
      if (block_index < SINGLE_INDIRECT) {
        ...
        unsigned char *block = disk + b_num * EXT2_BLOCK_SIZE;
        memcpy(block, target_name[block_index * EXT2_BLOCK_SIZE], EXT2_BLOCK_SIZE);
        indirect_block[block_index - 12] = (unsigned int)b_index;
        tar_inode->i_blocks += 2;
        block_index++;
      }
    }
  ...
  struct ext2_dir_entry_2 *target_entry = setup_entry((unsigned int)target_inode_num, target_name, 'l');
